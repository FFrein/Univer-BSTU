...
SOCKET cS;
																							...
																							//Если  поступившее сообщение является позывным сервера, то функция, заполняет возвращаемую структуру SOCKADDR_IN
bool GetServer(char* call, short port, sockaddr* from, int* flen)
{
	...
	return true;
																								...
																								//Функция предназначена для ожидания запроса клиентской программы
	...
	while (true)
	{
		...
		if (strcmp(name, bfrom) == 0)
		{
			return true;
																								...
																								//функция предназначена для пересылки позывного сервера программе клиента
																								//Функция предназначена для подтверждения сервером  запроса  клиента на установку соединения
																								//Функция оправляет в адрес клиента (параметры сокета клиента указываются в параметре to) свой позывной, что предполагает готовность сервера к дальнейшей работе с клиентом
bool PutAnswerToClient(
						char* name,																//[in] позывнйо сервера
