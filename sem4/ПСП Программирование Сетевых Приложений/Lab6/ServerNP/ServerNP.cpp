...
int main()
{
	...
	try
	{
		...
		if (!ConnectNamedPipe(
							sH,																														// [in] дескриптор именованного канала
							NULL																													// [in,out] используется для асинхр. связи
							)
			) {
		...
		cout << "Начинаем прослушку" << endl;
																																					...
																																					//Читать данные из канала																																// 2 и 3 блок - ввод и вывод данных(функции ReadFile и WriteFile) в именованном канале
		while (ReadFile(
						sH,																															// [in] дескриптор канала
						buffer,																														// [out] указатель на буфер ввода
						strlen(buffer - 1),																											// [in] количество читаемых байт
				...
				!= FALSE)
		{
			...
			WriteFile(sH, &buffer, strlen(buffer), &dwRead, NULL);
																																					...
																																					//Закончить обмен данными																																			// 4 блок - сервер разрывает соединение с помощью функции DisconnectNamedPipe и закрывает дескриптор именованного канала
